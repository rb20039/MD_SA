...
def main():
    ...
    for i in range(iterations):
        t = 1 - (0.001*float(i))
        candidate_domain = grid_update(rows, current_domain)
        candidate_eval = cost(columns, candidate_domain)
        if domain_eval - candidate_eval < 0:
            current_domain, current_eval = candidate_domain[:], candidate_eval
            domain, domain_eval = current_domain[:], candidate_eval
        elif (random.random() < math.exp((current_eval - candidate_eval) / (t+0.001))):
            current_domain, current_eval = candidate_domain[:], candidate_eval
                
        if domain_eval == 0 :
            break
    
    print(f"Current Evaluation: {domain_eval}")
    printng(domain)
